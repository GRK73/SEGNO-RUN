...
### 4. 설정 & 오프셋 캘리브레이션
- **환경설정 모달:** 메인 화면 우상단 ⚙ 버튼으로 접근.
- **오디오 오프셋 조절:** ±1ms/±5ms 버튼 및 슬라이더(-200ms~+200ms)로 정밀 조절. `localStorage`에 저장되어 영구 유지.
- **탭 캘리브레이션:** 120BPM 메트로놈에 맞춰 TAP하면 자동으로 오프셋을 계산.
- **전역 오프셋 적용:** `AudioManager.getCurrentTimeMS()`에서 오프셋을 적용하여 노트 스폰, 시각 이동, 판정 시스템이 모두 동일한 보정 시간을 사용.

### 5. 통합 채보 에디터 (Integrated Editor)
- **타임라인 편집:** BPM 기반 그리드 시스템 및 줌(Zoom) 기능. Web Audio API 적용으로 정밀하고 끊김 없는 재생/탐색 제공.
- **동적 타임라인:** 오디오 파일 길이에 맞춰 타임라인이 자동 확장 (기존 120초 고정 제한 해제).
- **노트 사운드 피드백:** 재생 중 시커가 노트를 지날 때 비프음 재생 (상단 1000Hz / 하단 700Hz). 롱노트는 duration 전체 길이만큼 지속음 출력.
- **메트로놈:** BPM에 맞춘 메트로놈 ON/OFF 토글. 4박 첫 비트 강세음 구분.
- **BPM 리캘큘레이션:** Old BPM → New BPM 변경 시 모든 노트의 time/duration을 비트 인덱스 기준으로 재계산.
- **시각적 편집:** 노트에 할당된 캐릭터 보유 색상이 인게임/에디터 양쪽에 즉각 반영.
...
├── components/         # React UI (SongSelect, ChartEditor, SettingsModal, MobileWarning 등)
├── assets/             # 정적 이미지 리소스
public/
└── assets/
    ├── audio/          # 오디오 파일 (.mp3, .ogg)
    └── charts/         # 채보 파일 (.json)
```
