...
- **Mouse Wheel:** 캐릭터 스위칭(Wheel Up/Down) 및 전용 스위치 노트 처리. (150ms 디바운스 적용)

### 2. 가변 로스터 및 캐릭터 시스템
- **2인의 캐릭터:** 코롯, 하노코. 각 캐릭터별 고유 색상 및 에셋 보유.
...
### 3. 정밀한 게임플레이 로직
- **롱노트(Long Note):** 홀딩 상태 실시간 렌더링, BPM 기반 틱 콤보 증가, 오토 컴플리트 및 조기 떼기 감지 기능.
...
- **점수 시스템:** PERFECT(300점), GREAT(200점) 기반. 피버 타임 중 1.5배 보너스. 실시간 점수 HUD 표시.
...
- **결과창 (Result Screen):** 스테이지 클리어 후 점수, 정확도(평균 편차 기반), 최대 콤보, 판정 세부 내역 표시.
- **등급 시스템:** 점수 기반의 S/A/B/C 등급 부여 및 등급별 전용 사운드 출력.
...
- **Session Management:** RETRY 시 `gameSessionId` 갱신으로 PixiCanvas 및 GameEngine 전체 초기화 후 재시작.
...
│   ├── HUDManager.ts    # 인게임 UI (아바타, 콤보, 점수, 인트로 텍스트, 피버 게이지)
...
│   └── images/          # 캐릭터 스프라이트 시트, 노트 이미지, 배경, 바닥 타일
│       └── coverimg/    # 앨범 커버 이미지 (PNG)
